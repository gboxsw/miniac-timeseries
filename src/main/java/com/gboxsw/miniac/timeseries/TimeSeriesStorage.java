...
public class TimeSeriesStorage {
    ...
    /**
     * Maximal number of decimal digits after decimal point of a item value
     * supported by the storage.
     */
    public static final int MAX_DECIMAL_PRECISION = 10;
    ...
    private static class Item {
         ...
         * @param decimalPrecision the number of decimal digits after decimal point.
         */
        Item(String name, byte decimalPrecision) {
            this.name = name;
            this.decimalPrecision = (byte) Math.min(Math.max(decimalPrecision, 0), MAX_DECIMAL_PRECISION);
    ...
    private class IndexTable {
        ...
        /**
         * Level of the index table. Index table of level 0 contains offsets of data blocks, i.e., index tables with
         * level 0 are leaf nodes in the index tree formed by index tables.
         */
        final int level;
         ...
         *
         * @param offset the offset of the index table.
         * @param level  the level of the index table.
         */
         ...
         *
         * @param newOffset the offset of the returned index table.
         ...
         *
         * @param storageFile the storage file open for writing.
    ...
    public class SampleReader implements AutoCloseable {
         ...
         *
         * @param fromTime initial time of reading.
         * @throws IOException if reading of the input failed.
         */
         ...
         * @throws RuntimeException if no sample is available.
         */
        ...
        public long getValueAsLong(String name) {
            ...
            int idx = findItemIndex(name);
            if (items[idx].decimalPrecision != 0) {
         ...
         * @throws IllegalArgumentException if item with required name does not exist.
         */
        ...
        private Number decodeValue(Item item, long rawValue) {
            if (rawValue == NULL_VALUE) {
                ...
                if (item.decimalPrecision == 0) {
                    ...
                    return ((double) rawValue) / item.decimalsPower;
                }
            }
        }
     ...
     */
    private final List<Sample> unflushedSamples = new ArrayList<>();
     ...
     * @throws IOException if reading of storage file failed.
     */
    ...
    /**
     * Constructs the storage, reads basic metadata from the file, and attempts
     * to correct the file if file is corrupted.
     *
     * @param file           the storage file.
     * @param autoCorrection indicates whether silent auto correction should be tried in
     *                       case of corrupted file is detected.
     * @throws IOException if reading of storage file failed.
     */
    public TimeSeriesStorage(File file, boolean autoCorrection) throws IOException {
        this.file = file;

        try (RandomAccessFile storageFile = new RandomAccessFile(file, autoCorrection ? "rw" : "r")) {
            boolean useSharedLock = !autoCorrection;
            FileLock lock = storageFile.getChannel().tryLock(0, Long.MAX_VALUE, useSharedLock);
            if (lock == null) {
            ...
            try {
                ...
                // initializes the storage time by scanning trailing block
                long uncorruptedFileSize = scanTralingBlock(storageFile, autoCorrection);

                if (autoCorrection) {
                    if (uncorruptedFileSize != fileSize) {
                        storageFile.setLength(uncorruptedFileSize);
                        fileSize = uncorruptedFileSize;
                    }
                }
            } finally {
                lock.release();
     ...
     * auto-flush feature is enabled.
     *
     * @param maxUnflushedSamples the maximal number of unflushed samples.
     */
     ...
     * auto-flush feature is enabled.
     *
     * @param maxSecondsBetweenFlushes the maximal time between two flushed in seconds.
     */
     ...
     * time point.
     *
     * @param fromTime the minimal time of samples.
     * @return the reader.
     * @throws IOException if reading from storage failed.
     */
     ...
     *
     * @throws IOException if reading from storage file failed.
     */
     ...
     *
     * @param storageFile the storage file open for reading.
    ...
    private void printIndexTableRecursively(IndexTable table, int indentation) {
        ...
        String indentationString = indentationBuilder.toString();
     ...
     *
     * @param time   the time of sample (arbitrary increasing non-negative value)
     * @param values the map with values of items forming the sample.
     * @throws IOException              if writing the sample failed.
     * @throws IllegalArgumentException if the sample is not complete (a missing value) or time of
     *                                  sample is invalid.
     */
     ...
     * @throws IOException if writing bytes to data output failed.
     */
     ...
     *
     * @param storageFile the storage file open for writing.
     * @param startTime   the start time of the block.
     * @throws IOException if the operation failed.
     */
    private void createNewBlock(RandomAccessFile storageFile, long startTime) throws IOException {
        // if the trailing index table is not full, append new index to data block
        ...
        // find the last free index table on the trailing branch
        IndexTable freeIndexTable = null;
        for (IndexTable indexTable : getTrailingBranch()) {
            if (!indexTable.isFull()) {
                freeIndexTable = indexTable;
            }
        }

        // if all index tables are full, it is necessary to create new main index table with greater level
        if (freeIndexTable == null) {
            increaseStorageIndexLevel(storageFile);
            freeIndexTable = mainIndexTable;
        }

        // construct new index tables along new trailing branch
        long startOffset = storageFile.length();
        IndexTable[] trailingBranch = new IndexTable[freeIndexTable.level + 1];
        trailingBranch[0] = freeIndexTable;
        ...
        for (int i = 1; i < trailingBranch.length; i++) {
            ...
            trailingBranch[i - 1].subtables[trailingBranch[i - 1].size - 1] = trailingBranch[i];
            startOffset += getRawIndexTableSizeInBytes();
            trailingBranch[i].appendIndex(startTime, startOffset);
        }

        // write newly created index tables first (in case of failure, index tree is not corrupted)
        for (int i = 1; i < trailingBranch.length; i++) {
            trailingBranch[i].write(storageFile);
        }

        // write updates in the index table where newly created trailing branch is starting
        trailingBranch[0].write(storageFile);

        trailingIndexTable = trailingBranch[trailingBranch.length - 1];
    }

    /**
     * Returns the path from main index table to the trailing index table, i.e., to index table indexing
     * the trailing block.
     *
     * @return the trailing branch in the index tree.
     */
    private IndexTable[] getTrailingBranch() {
    ...
    private void increaseStorageIndexLevel(RandomAccessFile storageFile) throws IOException {
        // create clone of the main index table at the end of the storage file
        IndexTable clonedMainTable = mainIndexTable.cloneWithNewOffset(storageFile.length());
        clonedMainTable.write(storageFile);

        // construct new main index table at offset of main index table
        IndexTable newMainTable = new IndexTable(mainIndexTable.offset, mainIndexTable.level + 1);
        newMainTable.appendIndex(mainIndexTable.startTimes[0], clonedMainTable.offset);
        newMainTable.write(storageFile);

        // update tree of index tables
     ...
     *
     * @param storageFile the storage file open for reading.
     * @param offset      the file offset of index table to be read.
     ...
     *
     * @param indexTable the search table.
     * @param time       the time.
     * @return the index.
     */
    ...
    /**
     * Scans trailing block to detected time of the last sample.
     *
     * @param storageFile    the storage file open for reading.
     * @param autoCorrection indicates whether auto-correction is required.
     * @return the offset of the next byte to be written to storage (expected
     * size of the storage file).
     */
    private long scanTralingBlock(RandomAccessFile storageFile, boolean autoCorrection) throws IOException {
        long trailerBlockOffset = trailingIndexTable.offsets[trailingIndexTable.size - 1];
        ...
        long uncorruptedFileSize = trailerBlockOffset;
        ...
        try {
            while (true) {
                ...
                if (recordType == RecordType.INDEX_TABLE) {
                    ...
                    time += readVLong(dataInput, byteCounter);

                    // skip item values
                    for (int i = 0; i < items.length; i++) {
                        readVLong(dataInput, byteCounter);
                    }
                } else if (recordType == RecordType.SAMPLE) {
                    time = readVLong(dataInput, byteCounter);
                ...
                uncorruptedFileSize = trailerBlockOffset + byteCounter.value;
            }
        } catch (Exception e) {
            if (autoCorrection) {
                return uncorruptedFileSize;
     ...
     *
     * @param file      the file of data storage.
     * @param items     the map where keys corresponds to names of item of samples.
     ...
     * @param blockSize the minimal length of a continuous data block.
     * @throws IOException if creation of storage failed.
     */
        ...
        try (RandomAccessFile storageFile = new RandomAccessFile(file, "rw")) {
            ...
            try {
                ...
                for (Map.Entry<String, Integer> entry : items.entrySet()) {
                    ...
                    decimals = Math.min(Math.max(decimals, 0), MAX_DECIMAL_PRECISION);
    ...
    /**
     * Helper class for counting read bytes.
     */
    ...
    /**
     * Byte counter used for reads with unspecified byte counter.
     */
    private static final ByteCounter FAKE_BYTE_COUNTER = new ByteCounter();
    ...
    private static long readVLong(DataInput in) throws IOException {
        return readVLong(in, FAKE_BYTE_COUNTER);
    }
     ...
     *
     * @param in          the data input.
     * @param byteCounter the counter of read bytes.
     * @return the retrieved value.
     * @throws IOException if read failed.
     */
    private static long readVLong(DataInput in, ByteCounter byteCounter) throws IOException {
